...
void showUsage(FILE *out) {
    ...
    fprintf(out, "  --bool              Enable boolean variables and logical operators & and |.\n");
    fprintf(out, "  --scopes            Enable the declaration of variables inside of compound statements.\n");
    fprintf(out, "  --ershov            Enable register optimization using the ershov number.\n");
    fprintf(out, "  --leafProc          Enables the optimization of leaf procedures.\n");
    fprintf(out, "  --records           Enable support for records.\n");
    fprintf(out, "  --valueSemantics    Enable value semantics.\n");
    fprintf(out, "  --compoundLiterals  Enable array and record literals.\n");
...
void checkOptionsIntegrity(CommandLineOptions *options) {
    if (options->compoundLiteralsEnabled && !options->valueSemanticsEnabled) {
...
void parseCommandLineOptions(CommandLineOptions *options, int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (strcmp(argv[i], "--leafProc") == 0) {
            options->leafProcedureOptimization = true;
        } else if (strcmp(argv[i], "--compoundLiterals") == 0) {
            options->compoundLiteralsEnabled = true;
        } else if (strcmp(argv[i], "--records") == 0) {
            options->recordsEnabled = true;
        } else if (strcmp(argv[i], "--valueSemantics") == 0) {
            options->valueSemanticsEnabled = true;
